package com.webdev.productsystem.Tours.TouristicLocation.Infrastructure.Hibernate;
...
public class TouristicLocationCustomTypes implements UserType {
    @Override
    public int[] sqlTypes() {
    ...
    public Object nullSafeGet(ResultSet rs, String[] names, SharedSessionContractImplementor session, Object owner) throws HibernateException, SQLException {
        List<TouristicLocationAddress> response = null;
        try {
            Optional<String> value = Optional.ofNullable(rs.getString(names[0]));
            if (value.isPresent()) {
                List<HashMap<String, Object>> objects = new ObjectMapper().readValue(value.get(), List.class);
                response = objects.stream()
                        .map(element -> new TouristicLocationAddress((String) element.get("id"), (String) element.get("data"), (String) element.get("zipCode"), (String) element.get("cityId")))
    ...
    public void nullSafeSet(PreparedStatement st, Object value, int index, SharedSessionContractImplementor session) throws HibernateException, SQLException {
        Optional<List<TouristicLocationAddress>> object = (value == null) ? Optional.empty() : (Optional<List<TouristicLocationAddress>>) value;
        try {
            if(object.isEmpty())
